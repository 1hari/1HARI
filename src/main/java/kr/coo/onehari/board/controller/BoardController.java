...
public class BoardController {
	...
	public @ResponseBody String get_board_info() {
		...
		if (!list.isEmpty() && list != null) {
			for (BoardType boardtype : list) {
				...
				obj.put("board_info_idx", boardtype.getBoardCode());
				obj.put("board_info_name", boardtype.getBoardName());
				root.add(obj);
			}
		} else {
	...
	public @ResponseBody String add_content(Board board) {
		System.out.println(board.toString());
		JSONObject root = board_service.add_content(board);
		System.out.println(root.toString());
		return root.toJSONString();
	}
