...
public class BoardService {
	...
	public JSONObject get_board_list(int boardNum, int page) {
		...
		for (Board board : list) {
			...
			obj.put("boardWriteDate", board.getBoardWriteDate());
			board_list.add(obj);
		}
...
//		content.setContent_file(filename);
		...
		if(result >0) {
			...
			root.put("boardNum", content.getBoardNum());
		}else {
	...
	// 컨텐츠 가져오기(Read)
	public JSONObject get_content(int boardNum) {
		BoardDao boarddao = sqlsession.getMapper(BoardDao.class);
		Board board = boarddao.getContent(boardNum);
		JSONObject root = new JSONObject();
		root.put("boardNum", boardNum);
		root.put("boardTitle", board.getBoardTitle());
		root.put("boardContent", board.getBoardContent());
		root.put("boardFileName", board.getBoardFileName());
		//root.put("content_writer_idx", board.getContent_writer_idx()); 우리 디비 테이블에 없음 
		root.put("writer", board.getWriter());
		root.put("boardWriteDate", board.getBoardWriteDate());
		return root;
	}
...
//	
//	// 컨텐츠 삭제하기
	public JSONObject delete_content(int boardNum) {
		BoardDao boarddao = sqlsession.getMapper(BoardDao.class);
		JSONObject root = new JSONObject();
